...
## read the data 
species_id = read.table(file = ('./data/Species_id.csv'), header = F, sep=",")
sample_id = read.table(file = ('./data/Sample_id.csv'), header = F, sep=",")
Ptrain = read.table(file = ('./data/Ptrain.csv'), header = F, sep=",")
ptst = read.table(file = ('./data/Ztest.csv'), header = F, sep=",")
# GCN_full = read.table(file = ('./data/GCN.csv'), header = F, sep=",")
# GCN_full = as.matrix(GCN_full)

## read the prediction of cNODE2
qtst = read.table(file = ('./results/qtst.csv'), header = F, sep=",")
qtrn = read.table(file = ('./results/qtrn.csv'), header = F, sep=",")
...
ggsave("my_plot.png", plot = g1, width = 6, height = 4, dpi = 300)
